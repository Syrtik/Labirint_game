...
class Player(GameSprite):
    ...
    def fire (self):
        bullet = Bullet(self.rect.right, self.rect.centery, 40, 40, 'bomb.png', 3 * 60)
        bullets.add(bullet)
    def update(self):
...
monster = Vrag (610, 190, 50, 50, 'bomb-4.png', 40)
monster2 = Vrag (610, 320, 50, 50, 'bomb-4.png', 60)
monster3 = Vrag (610, 270, 50, 50, 'bomb-4.png', 1.9 * 60)
walls = sprite.Group()
...
while run:
    for e in event.get():
        if e.type == QUIT:
            ...
            if e.key == K_a or e.key == K_LEFT or e.key == K_2:
                packman.x_speed = -2 
            elif e.key == K_d or e.key == K_RIGHT or e.key == K_1:
                packman.x_speed = 2
            elif e.key == K_w or e.key == K_UP or e.key == K_4:
                packman.y_speed = -2
            elif e.key == K_s or e.key == K_DOWN or e.key == K_3:
                packman.y_speed = 2
            elif e.key == K_SPACE:
                packman.fire()
        elif e.type == KEYUP:
    ...
    if finish == False:
        ...
        if sprite.spritecollide(packman, monsters, True):
            ...
            finish = True
        sprite.groupcollide(bullets, walls, False, False)
